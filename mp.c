...
void mp_boot_cpu( void *lapic, mp_cpu_entry_t *cpu, unsigned cpu_num ){
	display_puts( ">> trying to boot CPU " );
	...
	if ( cpu->flags & MP_CPU_FLAG_BOOTSTRAPPER ){
		...
		display_puts( ">> bootstrapping CPU is already running\n" );
	...
	*((volatile uint32_t *)BIOS_RESET_VECTOR) = (bootaddr & 0xff000) << 12;
...
mp_float_t *mp_find( void ){
	void *bios_ebda_rom = (void *)0xf0000;
...
void mp_enumerate( mp_float_t *mp ){
	...
	display_puts( ">> Found MP tables, trying to start APs...\n" );
	//hexdump( mp, 512 );
	...
	mp_config_t *config = (void *)mp->config_table;
	...
	void *lapic = (void *)config->lapic_addr;
